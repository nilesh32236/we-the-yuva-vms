...
import { istDayNumber } from '../kindness-challenge/date.utils';
...
export async function createStory(
  userId: string,
  data: { title: string; content: string; mediaUrl?: string; challengeId?: string; kindnessChallengeId?: string; isCompletion?: boolean },
  now: Date = new Date()
) {
  if (data.challengeId && data.kindnessChallengeId) {
    throw new AppError('Provide either challengeId or kindnessChallengeId, not both', 422);
  }

  // Handle kindness daily post (new flow) — server-derive day, validate 1..7 && <=currentDay
  if (data.kindnessChallengeId) {
    ...
    const isCompletion = !!data.isCompletion;
    const storyData = {
      title: stripHtml(data.title),
      content: stripHtml(data.content),
      mediaUrl: data.mediaUrl,
      userId,
      kindnessChallengeId: data.kindnessChallengeId,
      kindnessDay: day,
      isCompletion,
    };

    if (isCompletion) {
      // Completion must flip the challenge atomically — a non-transactional fallback would strand an orphan completion story.
      const prismaTx = prisma as unknown as { $transaction?: (fn: (tx: typeof prisma) => Promise<unknown>) => Promise<unknown> };
      if (typeof prismaTx.$transaction !== 'function') {
        throw new AppError('Completion stories require transaction support', 500);
      }
      const story = (await prismaTx.$transaction(async (tx) => {
        const s = await tx.story.create({ data: storyData });
