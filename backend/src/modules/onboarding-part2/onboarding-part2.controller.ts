...
export async function getPart2Handler(req: Request, res: Response, _next: NextFunction) {
  const result = await getPart2(req.user!.id, req.user!.role);
  res.json(result);
}

export async function putPart2Handler(req: Request, res: Response, _next: NextFunction) {
