...
export default function LevelRequestSuccessPage() {
  ...
  async function handleShare() {
    ...
    if (navigator.share) {
      await navigator.share({ title: 'Level Up!', text: `I just requested a level-up to ${detail?.requestedLevel ?? 'next level'} on WeTheYuva!`, url });
    } else {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <CelebrationOverlay levelName={detail?.requestedLevel ?? tierInfo.name} tier={detail?.tier ?? 1} points={detail?.points ?? 0} />
      ...
      <div className="bg-brand-surface rounded-2xl border border-brand-border p-8 text-center relative overflow-hidden">
        <div className="relative z-10 space-y-6">
          ...
          <p className="text-brand-muted text-sm max-w-sm mx-auto">
            Your level-up request for <span className="font-semibold text-brand-text">{detail?.requestedLevel ?? tierInfo.name}</span> has been sent for review. You&apos;ll be notified once it&apos;s approved.
          </p>
