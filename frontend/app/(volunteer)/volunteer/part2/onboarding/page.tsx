...
export default function Part2OnboardingPage() {
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem('part2-step', String(step));
    } catch {
      /* ignore */
    }
  }, [step]);
  ...
  const handleNext = async () => {
    if (await validateStep(step)) goToStep(step + 1);
  };
  ...
  if (challengeLoading || !gatingChecked)
    return (
      <div className="py-8">
        <SkeletonCard />
      </div>
      ...
      <div className="flex gap-1 overflow-x-auto pb-2" role="tablist">
        {STEPS.map((s, i) => (
          <Button
            ...
            className={`flex items-center gap-1 px-3 py-2.5 min-h-11 rounded-full text-xs font-medium whitespace-nowrap transition-colors cursor-pointer ${i === step ? 'bg-brand-primary text-white hover:bg-brand-secondary hover:text-white' : i < step ? 'bg-brand-primary/10 text-brand-primary hover:bg-brand-primary/20' : 'bg-brand-border/30 text-brand-muted cursor-not-allowed hover:bg-brand-border/30 hover:text-brand-muted'}`}
          >
      ...
      >
        ...
        <div className="flex gap-3 pt-2">
          ...
          {step < STEPS.length - 1 ? (
            ...
            >
              Submit Part II <Check className="w-4 h-4" />
            </Button>
          )}
        </div>
      </div>
