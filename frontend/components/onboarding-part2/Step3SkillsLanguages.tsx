...
export function Step3SkillsLanguages({ register, watch, setValue, errors }: StepProps) {
  ...
  const addLanguage = () => {
    ...
    setValue('languages', [...languages, { language: '', proficiency: 'BASIC' } as never], { shouldValidate: true });
  };
  ...
  return (
    <div className="space-y-5">
      <fieldset className="space-y-2">
        <legend className="text-sm font-medium text-brand-text">Life Skills * (select at least 2)</legend>
        <div className="grid grid-cols-1 gap-2">
          {LIFE_SKILLS.map((s) => (
            <label key={s} className="flex items-center gap-2 rounded-lg border border-brand-border px-3 py-2.5 min-h-11 cursor-pointer">
              <input type="checkbox" value={s} className="accent-brand-primary" {...register('lifeSkills')} />
      ...
      {showOther && (
        <div className="space-y-1.5">
          <label htmlFor="lifeSkillsOther" className="text-sm font-medium text-brand-text">Please specify other skill *</label>
